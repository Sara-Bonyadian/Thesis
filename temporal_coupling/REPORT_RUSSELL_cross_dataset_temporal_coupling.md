...
2. **Permutation-controlled group evidence is absent** - circular-shift null tests (`sig_group_perm_fdr`) did **not** survive FDR in **any** dataset x partition x pair (0/72 partition-pairs checked). Under the stricter evidence layer, we cannot claim group-level coupling beyond what the null would produce after lag search.
...
4. **Task and modality matter** - ds003838 **memory** shows the same broad pattern as rest but at **~3x weaker** peak |r|. HIIT **tetris** blocks are weaker than rest blocks. ds006848 (embedded PPG) is weaker and noisier than ds003838 rest (gold-standard ECG).
...
| HIIT-40 PPG | each of 4 partitions | 40 | 160 total (20 unique IDs x 8 sessions) |
...
20 participants x 4 conditions (pre/post x rest/tetris); PS and PH groups pooled per partition.
...
|------------|--------------|---------------|---------------------|
| ds003838 rest → ds003838 memory | Weaker (~3x) | Mostly same (EEG-leads HRV) | **Partial** |
...
5. **Multiple comparisons** - 9 pairs x 8 partitions x 3 datasets; focus on patterns, not individual p-values.
...
3. **Split HIIT by PS vs PH** (or pre vs post) before drawing intervention conclusions - pooled n=40 may hide condition x direction interactions already visible between pre_rest and post_rest.
